...
Each violation must include file (path in the repo). Include line as a non-negative integer when you can infer it from the diff; otherwise use null. suggestion must be concrete when you have one; otherwise use null.
...
Required JSON shape (always include line and suggestion on each violation; use null when not applicable):
{"violations":[{"severity":"info"|"warning"|"error","category":"smell"|"security"|"style"|"testing"|"convention","file":"string","line":number|null,"message":"string","suggestion":"string"|null}],"summary":"string","pass":boolean}
